...
export function AboutTrackPattern() {
  ...
  useEffect(() => {
    ...
    return scrollYProgress.on("change", (latest) => {
      ...
      // If we reach the end, set position to the last point of the path
      const finalPoint = activePath.getPointAtLength(pathLength);
      setPosition({
        x: clampedProgress === 1 ? finalPoint.x : point.x,
        y: clampedProgress === 1 ? finalPoint.y : point.y,
      });
    });
